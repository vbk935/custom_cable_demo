...
class WC_Shipping_UPS extends WC_Shipping_Method {
		...
		$this->instance_form_fields = array(
			...
			'negotiated' => array(
				...
				'description' => sprintf(
					...
					'<strong>', '</strong>',
					'<a href="http://www.ups.com/content/au/en/resources/techsupport/worldship/negotiated_rates.html">', '</a>'
				),
			),
			'signature' => array(
			...
			foreach ( $ups_responses as $code => $response ) {
				...
				if ( 1 == $xml->Response->ResponseStatusCode ) {
					...
					if ( get_woocommerce_currency() !== $currency ) {
						...
						$this->debug( sprintf( __( '[UPS] Rate for %1$s is in %2$s but store currency is %3$s.', 'woocommerce-shipping-ups' ), $rate_name, $currency, get_woocommerce_currency() ) );
						continue;
					}
	...
	private function per_item_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( $this->has_package_service_options() ) {
				...
				if ( $this->needs_delivery_confirmation() ) {
	 ...
	 */
	public function has_package_service_options() {
		return (
			$this->insuredvalue
			||
			$this->needs_delivery_confirmation()
		);
	}
