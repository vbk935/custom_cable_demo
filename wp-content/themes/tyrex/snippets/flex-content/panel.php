<?php
	$post_object = get_sub_field('panel_selector'); if( $post_object ): $post = $post_object; setup_postdata( $post ); 
...
<?php ///////////////////////////////////////// START PANEL ; ?>	
	<div class="panel panel-default">
		...
		<?php $background = get_field('background_color'); ?>
		<?php $text_color = get_field('text_color'); ?>
		
	
		<div class="panel-heading"<?php if($change_header == 1) { ?> style="background:<?php echo $background; ?>;"<? } ?>>
			<h2 class="panel-title"<?php if($change_header == 1) { ?> style="color:<?php echo $text_color; ?>;"<? } ?>><?php the_title(); ?></h2>
		</div>
...
<?php if($make_buttons != 1) {	?>			
				...
				<?php if( have_rows('link_list') ): while ( have_rows('link_list') ) : the_row(); ?>
					...
					<?php if(get_sub_field('link_type') == "anchor") { ?>
						...
						<li><a href="<?php the_sub_field('internal_page'); ?>#<?php echo $anchor; ?>"><?php the_sub_field('link_text'); ?></a></li>
					<?php } ?>						
...
<?php } else { ?>	
				...
				<?php if( have_rows('link_list') ): while ( have_rows('link_list') ) : the_row(); ?>
					...
					<?php if(get_sub_field('link_type') == "anchor") { ?>
						<?php $anchor = get_sub_field('internal_anchor'); ?>
						<a href="<?php the_sub_field('internal_page'); ?>#<?php echo $anchor; ?>" class="btn read-more"><?php the_sub_field('link_text'); ?></a>
					<?php } ?>						
